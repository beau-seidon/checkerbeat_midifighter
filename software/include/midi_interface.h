...
void pots_to_midi() {
    for(int i = 1; i < 3; i++){
        if(pots[i] != prev_pots[i]) {
            SerialMIDI.sendControlChange(POT_CC[i], pots[i], MIDI_CHANNEL);
            prev_pots[i] = pots[i];
        }
    }
}
...
//    if(js_changed(joystick_x, prev_joystick_x)) {
    ...
    if(joystick_y != prev_joystick_y) {
        SerialMIDI.sendControlChange(JS_CC[1], joystick_y, MIDI_CHANNEL); 
        ...
        if(i < 8) {
            if (shiftreg1 & 0x01 == 1) {
                if(!noteOn[i] && debounce(i)) {
                    ...
                    SerialMIDI.sendNoteOn((BUTTON_MAP[i]), DEFAULT_VELOCITY, MIDI_CHANNEL);
                ...
                if(noteOn[i] && debounce(i)) {
                    noteOn[i] = false;
                    SerialMIDI.sendNoteOff((BUTTON_MAP[i]), 0, MIDI_CHANNEL);
                    t1[i] = millis();
                }
            }
